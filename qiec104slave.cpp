...
QIec104::QIec104(int port, QMap<QString,  QMap<QString, config::dataConfig> > publishNode, MainWindow *window, modbus_mapping_t *mapping, QMap<QString, QList<int>> typeAddress, QObject *parent) : QObject(parent) {
    this->end = false;
    ...
    this->window = window;
    this->publishDatas = publishNode;
    connect(this->tm, &QTimer::timeout, this, &QIec104::slotTimeOut);

    initServer();//作为从站 初始化服务器
}

void QIec104::initServer() {
    char buf[100];
    tcpServer = new QTcpServer(this);//监听本地的port 所有对port的连接都会触发newconnect
    ...
    // use the second non-localhost IPv4 address
    int j = 0;
...
void QIec104::slotTcpReadyRead() {
    int i = 0;
    if (cli->bytesAvailable() < 20) {
        int porchone = QDateTime::currentDateTime().toMSecsSinceEpoch();
        while (!cli->waitForReadyRead(100) && i < 5) {  // delay 0.6 sec
            // readyRead() signal not be emitted
            ++i;
        }
        int porchoTwo = QDateTime::currentDateTime().toMSecsSinceEpoch();
        qDebug() << "before " << porchone << " after" << porchoTwo;
        qDebug() << " differ " << porchone - porchoTwo;

    }
    messageReadyRead(typeAddress);
}
...
void QIec104::slotTcpError(QAbstractSocket::SocketError err) {
    ...
    // TODO:
    char buf[100];
    sprintf(buf, "socket error : %d(%s)", err, cli->errorString().toStdString().c_str());
    // log.pushMsg(buf);
    qDebug() << buf;
    //    }
}
