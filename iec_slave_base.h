#ifndef IEC_SLAVE_BASE_H
#define IEC_SLAVE_BASE_H

#include "iec_slave_def.h"
// #include "logmsg.h"
#include <sstream>
#include <string.h>
...
public:
    ...
    //单点遥控确认返校
    void singleYkCon();
    modbus_mapping_t *mapping; //存放解析读到的数据的map
    ...
    static const uint32_t M_PS_NA_1 = 20;  // Packed single point information with status change detection
    static const uint32_t M_SP_TB_1 = 30;  // single-point information with time tag(cp56time2a)
    static const uint32_t M_DP_TB_1 = 31;  // double-point information with time tag
    static const uint32_t M_ST_TB_1 = 32;  // step position information with time tag
    ...
    static const uint32_t M_EP_TD_1 = 38;  // Event of protection equipment with CP56Time2a time tag
    static const uint32_t M_EP_TE_1 = 39;  // Packed start events of protection // equipment with CP56Time2a time tag
    static const uint32_t M_EP_TF_1 = 40;  // Packed output circuit information of protection equipment with// CP56Time2a time tag
    ...
    static const uint32_t C_SE_NC_1 = 50;  // set-point short floating point command
    ...
    static const uint32_t C_SE_TA_1 = 61;  // set-point normalised command with time tag
    static const uint32_t C_SE_TB_1 = 62;  // set-point scaled command with time tag
    static const uint32_t C_SE_TC_1 = 63;  // set-point short floating point command with time tag
    static const uint32_t C_BO_TA_1 = 64;  // Bitstring of 32 bit command with time tag
    ...
    static const uint32_t C_TS_TA_1 = 107;  // test command with time tag CP56Time2a
    static const uint32_t P_ME_NA_1 = 110;  // Parameter of measured values, normalized value
    static const uint32_t P_ME_NB_1 = 111;  // Parameter of measured values, scaled value
    static const uint32_t P_ME_NC_1 = 112;  // Parameter of measured values, short floating point number
...
public:
    ...
    void messageReadyRead(QMap<QString, QList<int>> typeAddress);
    void showMessage(const char *buf, int size, bool isSend);

    int m_parMax = 48;//有序遥测 float 最多一次发送48个 ，刚好总长度是255
    int m_yxMax = 90;//有序遥信 char 最多一次发送48个 ，刚好总长度是255
...
private:
    ...
    void sendYX(uint8_t type, bool sq,   QMap<QString, QList<int>>  typeAddress); // 模拟并发送遥信数据
    void sendYK(uint8_t type, bool sq,   QMap<QString, QList<int>>  typeAddress); // 模拟并发送遥控数据
    void sendYC(uint8_t type, bool sq,   QMap<QString, QList<int>>  typeAddress); // 模拟并发送遥测数据
    void sendYD(uint8_t type, bool sq,   QMap<QString, QList<int>>  typeAddress); // 模拟并发送遥调数据
