#ifndef IEC_SLAVE_DEF_H
#define IEC_SLAVE_DEF_H

#include <cstdint>

#pragma pack(push)
#pragma pack(1)
// struct IEC104_mapping {
//     int single_yx_bits;
//     int single_yx_start_bits;
//     int double_yc_bits;
//     int double_yc_start_bits;
//     int yc_bytes;
//     int yc_start_bytes;
//     uint8_t *tab_single_yx_bits;
//     uint8_t *tab_double_yx_bits;
//     uint16_t *tab_yc_bytes;
// } ;
...
struct apdu {
    ...
    union {
        ...
        struct {
            uint16_t ioa16;//地址应该和sq1的要隔开
            uint8_t ioa8;
            struct iec_type13 obj[48];
        } sq13;//连续值 只要一个点号，占5n+3个字节，一次最多发送48个

        struct {
            uint16_t ioa16;
            uint8_t ioa8;
            struct iec_type13 obj;
        } nsq13[29];//不连续值 每一个都有点号，占8个字节 一次最多发送30个
