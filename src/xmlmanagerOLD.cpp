...
//                     nodename = xmlreader.name().toString();
//                     bool start = xmlreader.isStartElement();
...
// QMap<QString, QMap<int, config::publishNode>> XmlManager::loadPublishXmlFile(QFile &file, QString *publishIp, int *publishPort, QString *masterIp) {
//     QString nodename;  // 节点的名字
...
//                 *publishPort = xmlreader.attributes().value("port").toInt();
//             }
//             if ((nodename == "node") &&
//                     xmlreader.isStartElement())  // 如果是第一个子节点的开始执行下列代码
//             {
//                 int slaveId =  xmlreader.attributes().value("slave_id").toInt();
//                 nodename = xmlreader.name().toString();
...
//                 publishNode.slave_id = xmlreader.attributes().value("slave_id").toInt();
//                 while (!(nodename == "node" &&
//                          xmlreader.isEndElement()))  // 如果子节点中的内容没有结束
//                 {
//                     xmlreader.readNextStartElement();
...
//                         data.IEC104Type = xmlreader.attributes().value("IEC104Type").toString();
...
//                     modbus_publish_datas.insert(slaveId, publishNode);
//                 }
//                 if (templateName == "IEC104")
//                 {
//                     IEC104_publish_datas.insert(slaveId, publishNode);
//                 }
